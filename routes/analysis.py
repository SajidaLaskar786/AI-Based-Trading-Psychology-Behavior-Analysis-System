...
def prepare_round_trip_df(file_bytes: bytes, filename: str) -> pd.DataFrame:
    ...
    if "buy_value" in df.columns:
        buy_val = pd.to_numeric(df["buy_value"], errors="coerce").fillna(0.0)
    else:
    ...
    if "sell_value" in df.columns:
        sell_val = pd.to_numeric(df["sell_value"], errors="coerce").fillna(0.0)
    else:
    ...
    if "pnl" in df.columns:
        pnl_val = pd.to_numeric(df["pnl"], errors="coerce").fillna(0.0)
    else:
